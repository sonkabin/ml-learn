...
'''
模型树与回归树比较
trainMat = np.mat(regTree.loadDataSet('mycode/Ch09/bikeSpeedVsIq_train.txt'))
testMat = np.mat(regTree.loadDataSet('mycode/Ch09/bikeSpeedVsIq_test.txt'))
reTree = regTree.createTree(trainMat, ops=(1, 20))
yHat = regTree.createForecast(reTree, testMat[:, 0])
np.corrcoef(yHat, testMat[:, 1], rowvar=0)[0, 1]

modelTree = regTree.createTree(trainMat, regTree.modelLeaf, regTree.modelErr, ops=(1, 20))
yHat = regTree.createForecast(modelTree, testMat[:, 0], regTree.modelTreeEval)
np.corrcoef(yHat, testMat[:, 1], rowvar=0)[0, 1]

ws = regTree.linearSolve(trainMat)[0]
